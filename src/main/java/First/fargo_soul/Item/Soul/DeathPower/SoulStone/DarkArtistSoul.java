...
public class DarkArtistSoul extends SoulItem {
    ...
    public static void DarkArtistSoulDamageHandler(LivingIncomingDamageEvent event) {
        if (event.getSource().getEntity() instanceof ServerPlayer player && CurioUtils.isEquipped(player, Souls.DarkArtistSoul.get()) && event.getEntity() instanceof LivingEntity livingEntity && event.getSource().getWeaponItem() != null) {
    ...
    public static void DarkArtistSoulRenderHnadler(SlotContext slotContext, PoseStack poseStack, MultiBufferSource renderTypeBuffer, float ageInTicks) {
        if (slotContext.entity() instanceof LocalPlayer player && CurioUtils.isEquipped(player, Souls.DarkArtistSoul.get())) {
