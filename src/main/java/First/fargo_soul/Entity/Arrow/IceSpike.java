...
public class IceSpike extends ThrowableItemProjectile {
    ...
    public IceSpike(Level level, LivingEntity shooter) {
        super(EntityRegister.IceSpike.get(), shooter, level);
    }

    public IceSpike(Level level, double x, double y, double z) {
        super(EntityRegister.IceSpike.get(), x, y, z, level);
    }

    @Override
    protected @NotNull Item getDefaultItem() {
        return BaseItemsRegister.IceSpike.get();
    }
    ...
    protected void onHitEntity(@NotNull EntityHitResult result) {
        ...
        if (result.getEntity() instanceof LivingEntity livingEntity) {
            ...
            if (!CurioUtils.isEquipped(livingEntity, SoulsRegister.FrostSoul.get())) {
