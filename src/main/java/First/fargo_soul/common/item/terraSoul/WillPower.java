...
public class WillPower extends SoulItem {
    ...
    public void hurt(LivingIncomingDamageEvent event) {
        if (event.getSource().getEntity() instanceof LivingEntity attacker && !attacker.level().isClientSide()) {
            if (CurioUtils.isEquipped(attacker, WillPower.class)) {
                SoulAbilityData.SoulInfo soulInfo = SoulAbilityData.getSoulInfo(attacker, WillPower.class);
    ...
    public void tick(LivingEntity ticker) {
        if (!ticker.level().isClientSide() && CurioUtils.isEquipped(ticker, WillPower.class)) {
            SoulAbilityData.SoulInfo soulInfo = SoulAbilityData.getSoulInfo(ticker, WillPower.class);
