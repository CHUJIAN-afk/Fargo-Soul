    ...
    public static class Event {
        ...
        public static void LavaSoulTickHandler(EntityTickEvent.Post event) {
            if (event.getEntity() instanceof LivingEntity attacker && !attacker.level().isClientSide()) {
                if (CurioUtils.isEquipped(attacker, LavaSoul.class) && attacker.tickCount % 5 == 0 && SoulUtils.getSoulTarget(attacker, 10) instanceof LivingEntity soulTarget) {
                    soulTarget.setRemainingFireTicks(Math.min(soulTarget.getRemainingFireTicks() + 10, 80));
                    if (CurioUtils.isEquipped(attacker, NaturePower.class) && attacker.getRandom().nextDouble() < 0.05) {
