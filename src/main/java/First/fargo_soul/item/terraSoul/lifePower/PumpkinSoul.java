    ...
    public static class Event {
        ...
        public static void Tick(EntityTickEvent.Post event) {
            if (event.getEntity() instanceof LivingEntity attacker && !attacker.level().isClientSide()) {
                if (CurioUtils.isEquipped(attacker, PumpkinSoul.class)) {
                    ...
                    if (level.getBlockState(below).getBlock() instanceof PumpkinBlock) {
                        ...
                        for (LivingEntity target : targetList) {
                            SoulUtils.attack(attacker, target, DamageTypes.EXPLOSION, CurioUtils.isEquipped(attacker, LifePower.class) ? 6 : 3);
