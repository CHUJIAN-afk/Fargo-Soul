...
/*
	public static void CelestialShellTickHandler(PlayerTickEvent.Post event) {
		if (event.getEntity() instanceof ServerPlayer player) {
			...
			if (player.tickCount % 20 == 0 && CurioUtils.isEquipped(player, ItemRegister.CelestialShell.get())) {
	...
	public static void CelestialShellBreathHandler(LivingBreatheEvent event) {
		if (event.getEntity() instanceof Player player && CurioUtils.isEquipped(player, ItemRegister.CelestialShell.get())) {
	...
	private static void CelestialShellAddAttributeModifiers(ServerPlayer player, Holder<Attribute> attributeHolder, double amount, AttributeModifier.Operation operation) {
		AttributeUtils.ConditionAttributeModifier(player, attributeHolder, ItemRegister.CelestialShell.getId(), amount, operation, CurioUtils.isEquipped(player, ItemRegister.CelestialShell.get()));
	}
	...
	private static void CelestialShellNightAddAttributeModifiers(ServerPlayer player, Holder<Attribute> attributeHolder, double amount, AttributeModifier.Operation operation) {
		AttributeUtils.ConditionAttributeModifier(player, attributeHolder, ItemRegister.CelestialShell.getId(), amount, operation, CurioUtils.isEquipped(player, ItemRegister.CelestialShell.get()) && player.serverLevel().isNight());
	}
*/
